...
namespace Marisa.Application.ServicesTests.UserServiceTests
{
    public class UserAuthenticationServiceTest
    {
        ...
        [Fact]
        public async Task Should_ChangePasswordUser_Successfully()
        {
            var userChangePasswordDTO = new UserChangePasswordDTO("augusto@gmail.com", "password123");

            _userAuthenticationServiceConfiguration.UserRepositoryMock.Setup(rep => rep.GetUserByEmailInfoUpdate(It.IsAny<string>()))
                .ReturnsAsync(new User());

            var byteUser = new byte[] { 1, 2, 3, 4, 5, 6, 7, 8 };

            _userAuthenticationServiceConfiguration.UserCreateAccountFunctionMock.Setup(valid => valid.GenerateSalt())
               .Returns(byteUser);

            _userAuthenticationServiceConfiguration.UserCreateAccountFunctionMock.Setup(valid => valid.HashPassword(It.IsAny<string>(), It.IsAny<byte[]>()))
               .Returns("asdcasfcDFOJJIO2323IUIJOZ");

            _userAuthenticationServiceConfiguration.UserRepositoryMock.Setup(valid => valid.UpdateAsync(It.IsAny<User>()))
               .ReturnsAsync(new User());
            ...
            if (data != null)
            {
                Assert.True(data.PasswordUpdateSuccessfully);
            }
        }

        [Fact]
        public async Task Should_Return_Null_When_Get_User_ChangePasswordUser()
        {
            var userChangePasswordDTO = new UserChangePasswordDTO("augusto@gmail.com", "password123");
        ...
        [Fact]
        public async Task Should_Return_Null_When_Update_User_Repository_ChangePasswordUser()
        {
            var userChangePasswordDTO = new UserChangePasswordDTO("augusto@gmail.com", "password123");
